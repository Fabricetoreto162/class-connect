...
if (isset($_POST["connexion"])) {
    if (!empty($_POST['email']) && !empty($_POST['password'])) {
        ...
        $connecter_etudiant= $connecter->prepare("SELECT * FROM users WHERE Email = ?");
        $connecter_etudiant->execute([$email]);
        $resultat_etudiant=$connecter_etudiant->fetch();



        if ($resultat_etudiant) {
            $email_defaut = $resultat_etudiant["Email"];
            $password_defaut = $resultat_etudiant["Password"];
            $role_defaut = $resultat_etudiant["Role"];
            $nom = $resultat_etudiant["Nom"];
            $prenom = $resultat_etudiant["Prenom"];

            // Vérification des informations
            if ($email_defaut == $email && password_verify($password_input, $password_defaut) && $role_defaut == "etudiant") {
              ...
              <div class="card card-plain">
                ...
                <div class="card-body">
                  <form role="form" method="post" action="">
                    ...
                    <div style="color: red; font-size:20px;"><?=$msg2?></div>
                ...
                <div class="card-footer text-center py-0 px-lg-2 px-1">
                  <p class="my-2 text-sm mx-auto">
                    ...
                    <a href="inscription-etudiant.php" class="text-warning mx-2 Connexion font-weight-bold">Inscription</a>
                  </p>
                </div>
              </div>
            </div>
