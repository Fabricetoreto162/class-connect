...
if (isset($_POST['save_edict'])) {
    ...
    $contenu_du_cours = verification_input($_POST['contenu_du_cours']);
    $objectifs = verification_input($_POST['objectifs']);
    $objectifAtteint = verification_input($_POST['objectifAtteint']);
    $travail = verification_input($_POST['travail']);
    $avis_remarques = verification_input($_POST['avis_remarques']);
    

    $updateQuery = "
        UPDATE teacher_notebooks 
        SET contenu_du_cours = :contenu_du_cours, 
            objectifs = :objectifs, 
            objectifAtteint = :objectifAtteint, 
            travail = :travail, 
            avis_remarques = :avis_remarques
        WHERE note_id = :note_id AND teacher_id = :teacher_id
    ";

    $updateStmt = $connecter->prepare($updateQuery);
    $updateStmt->bindParam(':contenu_du_cours', $contenu_du_cours);
    $updateStmt->bindParam(':objectifs', $objectifs);
    $updateStmt->bindParam(':objectifAtteint', $objectifAtteint);
    $updateStmt->bindParam(':travail', $travail);
    $updateStmt->bindParam(':avis_remarques', $avis_remarques);
    $updateStmt->bindParam(':note_id', $note_id);
    $updateStmt->bindParam(':teacher_id', $teacher_id);

    $updateStmt->execute();

    header("Location:cahier-texte.php");
    exit();
}
...
<div class="modal fade" id="modalModifierCours" tabindex="-1" aria-labelledby="modalModifierCoursLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="form-modifier-cahier" method="POST" >
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Annuler</button>
                        <button type="submit" name="save_edict" class="btn btn-success">
