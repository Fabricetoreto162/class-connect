...
if (isset($_POST["connexion"])) {
    if (!empty($_POST['email']) && !empty($_POST['password'])) {
        ...
        $connecter_enseignant= $connecter->prepare("SELECT * FROM users WHERE Email = ?");
        $connecter_enseignant->execute([$email]);
        $resultat_enseignant = $connecter_enseignant->fetch();



        if ($resultat_enseignant) {
            $email_defaut = $resultat_enseignant["Email"];
            $password_defaut = $resultat_enseignant["Password"];
            $role_defaut = $resultat_enseignant["Role"];
            $nom = $resultat_enseignant["Nom"];
            $prenom = $resultat_enseignant["Prenom"];

            // Vérification des informations
            if ($email_defaut == $email && password_verify($password_input, $password_defaut) && $role_defaut == "enseignant") {
              ...
              <div class="card card-plain">
                ...
                <div class="card-footer text-center py-0 px-lg-2 px-1">
                  <p class="my-2 text-sm mx-auto">
                    ...
                    <a href="inscription-enseignant.php" class="text-warning mx-2 Connexion font-weight-bold">Inscription</a>
                  </p>
                </div>
              </div>
            </div>
